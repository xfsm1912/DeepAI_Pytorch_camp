...
# @author     : Jianhua Ma
# @date       : 20210405
# @brief      : rnn classifies human names
"""
from io import open
import glob
import unicodedata
...
def randomTrainingExample():
    category = randomChoice(all_categories)                 # select category
    line = randomChoice(category_lines[category])           # select one sample
...
if __name__ == "__main__":
    ...
    if not os.path.exists(data_dir):
        raise Exception("\n{} not exist, please download rnn_data and place it in \n{}".format(
            data_dir, os.path.dirname(data_dir)))

    path_txt = os.path.join(data_dir, "*.txt")
    all_letters = string.ascii_letters + " .,;'"
    n_letters = len(all_letters)    # 52 + 5 total character number
    print_every = 5000
    plot_every = 5000
    learning_rate = 0.005
    n_iters = 200000
    ...
    if not os.path.exists(path_model):
        raise Exception("\n{} not exist，please download rnn_state_dict.pkl and place it in the \n{}".format(
