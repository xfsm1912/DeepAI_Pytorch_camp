...
def gen_data(num_data=10, x_range=(-1, 1)):
    ...
    train_y = w * train_x + torch.normal(mean=0, std=0.5, size=train_x.size())
    test_x = torch.linspace(*x_range, num_data).unsqueeze_(1)
    test_y = w * test_x + torch.normal(mean=0, std=0.3, size=test_x.size())
...
for epoch in range(max_iter):
    ...
    # backward
    loss_normal, loss_wdecay = loss_fuc(pred_normal, train_y), loss_fuc(pred_wdecay, train_y)
